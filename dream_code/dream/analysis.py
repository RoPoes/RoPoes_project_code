...
def analyze_ndds_dataset(
    ...
    if pnp_analysis:
        all_gt_kp_positions = np.array(all_gt_kp_positions)   #all_gt_kp_positions - gt kp in 3d
        camera_K = dream.utilities.load_camera_intrinsics(
            found_ndds_dataset_config["camera"]
        )
        #kp_projs_est -- 2d kp from hourglass network

        for kp_projs_est, kp_projs_gt, kp_pos_gt in zip(
            all_kp_projs_detected_raw, all_kp_projs_gt_raw, all_gt_kp_positions
        ):

            n_inframe_projs_gt = 0  #no of projected gt kp within image res
            ...
            idx_good_detections = np.where(kp_projs_est > -999.0)  #tuple of 2 arrays where arr1[0], arr2[0] will be one such point >-999.0
            idx_good_detections_rows = np.unique(idx_good_detections[0])
            kp_projs_est_pnp = kp_projs_est[idx_good_detections_rows, :]  #take unique rows which are there in first array of np.where
            #take unique rows where atleast one element is > -999.0
            ...
            if pnp_retval:
                ...
                )   #this is error : kp_pos_aligned - keypoint_positions_wrt_cam_gt
            else:
