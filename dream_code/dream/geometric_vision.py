...
def solve_pnp(
    canonical_points,   #3d points after filtering rows based on -999.0
    projections,     #2d kp est points after filtering rows based on -999.0
    camera_K,
...
def add_from_pose(translation, quaternion, keypoint_positions_wrt_cam_gt, camera_K):
    ...
    #transform is transformation (4*4) matrix
    kp_pos_gt_homog = np.hstack(     #13*4   -- these are 3d points in camera system ????
        (
    ...
    kp_pos_aligned = np.transpose(np.matmul(transform, np.transpose(kp_pos_gt_homog)))[
        :, :3
    ]   #13*3   , these are 3d points in world system
