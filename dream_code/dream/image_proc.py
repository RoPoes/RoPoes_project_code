...
def peaks_from_belief_maps(belief_map_tensor, offset_due_to_upsampling):
    ...
    thresh_map_after_gaussian_filter = 0.001
    sigma = 3

    all_peaks = []
    peak_counter = 0

    for j in range(belief_map_tensor.size()[0]):   #loop thru 8 belief maps in belief map tensor
        belief_map = belief_map_tensor[j].clone()
        map_ori = belief_map.cpu().data.numpy()

        #apply gaussian filter
        map = gaussian_filter(map_ori, sigma=sigma)   #same size as belief map
        print
        p = 1
        ...
        ran = win // 2
        peaks_avg = []
        for p_value in range(len(peaks)):   #for every keypoint in belief map
            p = peaks[p_value]
            weights = np.zeros((win, win))
            i_values = np.zeros((win, win))
            j_values = np.zeros((win, win))
            for i in range(-ran, ran + 1):     #[-2 to 2]
                for j in range(-ran, ran + 1):      #[-2 to 2]
                    if (
                        p[1] + i < 0
