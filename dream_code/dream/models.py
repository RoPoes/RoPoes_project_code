...
class DreamHourglass(nn.Module):
    ...
    def forward(self, x):
        # lets say batch size  = 4, x shape: (4, 3, 400, 400) : (400, 400, 3) are dims wrt model input

        # Encoder    (we have 4 encoder blocks where each one is seperated by maxpooling layer and this decreases the spatial resolution of feature map by half)
        x_0_1 = self.layer_0_1_down(x)  #conv2d(3, 64) + relu + conv2d(64, 64) +relu   --> (4, 64, 400, 400)
        x_0_1_d = self.down_sample(x_0_1)  #maxpooling(2)   --> (4, 64, 200, 200)
        x_0_2 = self.layer_0_2_down(x_0_1_d)   #conv2d(64, 128) + relu + conv2d(128, 128) +relu  --> (4, 128, 200, 200)
        x_0_2_d = self.down_sample(x_0_2)   #maxpooling(2)   --> (4, 128, 100, 100)
        x_0_3 = self.layer_0_3_down(x_0_2_d)   #conv2d(128, 256) + relu + conv2d(256, 256) +relu + conv2d(256, 256) + relu + conv2d(256, 256) +relu --> (4, 256, 100, 100)
        x_0_3_d = self.down_sample(x_0_3)  #maxpooling(2)   --> (4, 256, 50, 50)
        x_0_4 = self.layer_0_4_down(x_0_3_d)   #conv2d(256, 512) + relu + conv2d(512, 512) +relu + conv2d(512, 512) + relu + conv2d(512, 512) +relu  --> (4, 512, 50, 50)
        x_0_4_d = self.down_sample(x_0_4)   #maxpooling(2)   --> (4, 512, 25, 25)
        x_0_5 = self.layer_0_5_down(x_0_4_d)   #conv2d(512, 512) + relu + conv2d(512, 512) +relu + conv2d(512, 512) + relu + conv2d(512, 512) +relu  --> (4, 512, 25, 25)
        ...
        if self.deconv_decoder:
            ...
            y_0_5 = self.upsample_0_4(decoder_input)    #decoder_input is output of encoder  --> (4, 512, 25, 25)
            # Upsample(scale_factor=2)) + Conv2d(512, 256) + ReLU + Conv2d(256, 256)
            # y_0_5: (4, 256, 50, 50)
            if self.skip_connections:
                y_0_out = self.upsample_0_3(y_0_5 + x_0_3_d)
            else:
                y_0_out = self.upsample_0_3(y_0_5)
            # Upsample(scale_factor=2)) + Conv2d(256, 128) + ReLU + Conv2d(128, 64)
            # y_0_out: (4, 64, 100, 100)
