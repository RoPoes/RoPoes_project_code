...
class DreamNetwork:
    ...
    def keypoints_from_image(
        ...
        # do preprocessing    ('resize' by def in architecture of config file)
        image_preprocessing = (
            image_preprocessing_override
            if image_preprocessing_override
            else self.image_preprocessing()
        ...
        )  #(kps wrt output image from network (208, 208))
        ...
        # Convert keypoints from net-output to net-input (kps wrt input image to network (400, 400))
        detected_kp_projs_net_in = dream.image_proc.convert_keypoints_to_netin_from_netout(
            detected_kp_projs_net_out, netout_res_inf  #(208, 208)
            ,netin_res_inf
        )
        detected_kp_projs = dream.image_proc.convert_keypoints_to_raw_from_netin(
            detected_kp_projs_net_in,
            netin_res_inf,   #(400, 400)
            input_image_resolution,  #(640, 480)
            image_preprocessing,
        )   #(kps wrt actual(original) image  (640, 480))
    ...
    def inference(self, network_input):
        ...
        if self.architecture_type == "dream_legacy":
            ...
            network_head_output = self.model(network_input)    #network input: list of single elemen of size torch.Size([1, 3, 400, 400])

            # network_head_output is list (single element)
            # network_head_output[0].shape   :   torch.Size([1, 8, 208, 208])

            # use the last layer  (whats this?)
            belief_maps_batch = network_head_output[-1]    #torch.Size([1, 8, 208, 208])
            #8 is no of potential keypoints (in case of iiwa arm)
            ...
            for belief_maps in belief_maps_batch:     #loop thru each belief map
                #whats purpose of below fn?
                peaks = dream.image_proc.peaks_from_belief_maps(
                    belief_maps, **peaks_from_belief_maps_kwargs
                )
                #peaks is list of size 8, 
                # ex: peaks[0]
                # [(105.27422172114204, 139.52105799237165, 0.4768089, 0)]
                # peaks[4]
                # [(103.29822856533576, 58.46904031069006, 0.8529741, 4), (106.50456163214193, 155.63474048303564, 0.08913552, 5)]        
                    ...
                    if len(peak) == 1:
                        ...
                        if self.use_belief_peak_scores and len(peak) > 1:
                            ...
                            if (
                                ...
                                # detected_kp_projs.append(
                                #     [
                                #         peak_sorted_by_score[1][0],
                                #         peak_sorted_by_score[1][1],
                                #     ]
                                # )
                            else:
                        ...
                        else:    #we add (-999, -999) when no kps are there in belief map
                            # Can't determine -- return no detection
                            # Can't use None because we need to return it as a tensor
                            detected_kp_projs.append([-999.999, -999.999])
            ...
            #detected_kp_projs_batch is list where no of elements = no of elements in belief maps batch (1 here)
            network_output = [belief_maps_batch, detected_kp_projs_batch]
