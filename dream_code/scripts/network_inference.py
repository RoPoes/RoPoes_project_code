...
def network_inference(args):
    ...
    orig_image_dim = tuple(image_rgb_OrigInput_asPilImage.size)    #(640, 480) -- y not 3 channels?
    ...
    else:     #in architecture of config file, we have 'resize'
    ...
    #kps wrt original image : {"detected_keypoints": detected_kp_projs}
    ...
    kp_coords_wrtNetOutput_asArray = detection_result["detected_keypoints_net_output"]   #key points wrt network output image(208, 208)
    image_rgb_NetInput_asPilImage = detection_result["image_rgb_net_input"]
    input_image_dim = image_rgb_NetInput_asPilImage.size   #(400, 400)
    belief_maps_wrtNetOutput_asTensor = detection_result["belief_maps"]
    kp_coords_wrtNetInput_asArray = detection_result["detected_keypoints_net_input"]   #key points wrt network input image(400, 400)
    ...
    print("Done.")
    return detection_result["detected_keypoints"], dream_network.keypoint_names
