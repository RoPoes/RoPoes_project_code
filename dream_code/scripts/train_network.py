...
def train_network(args):
    ...
    if args.resume_training:
        ...
        train_log = {
            "epochs": [],
            "losses": [],   #this is error in each epoch on training data
            "validation_losses": [],
            "batch_training_losses": [],   #errors for all batches in epoch
    ...
    # Parse input data
    input_data_path = args.input_data_path    #path to training images along with their json file
    ...
    if (    #check if input data path is absolute path
    ...
    found_data_config = found_data[1]   #(config files like camera_settings, object_settings.json files)
    ...
    manipulator_config = manipulator_config_file["manipulator"]   #YAML will prase json file as ordered dict of tuples (where each tuple is treated as key val pair)
    ...
    network_config = odict(    #this will have input_data_dir_path, manipulator/arm config, training network architecture config, training details config.
        [
    ...
    for e in tqdm(range(start_epoch, args.epochs)):
        ...
        dream_network.enable_training()   #setting optimizer from command line args

        training_batch_losses = []
        training_batch_sample_names = []
        #sample batches in each epoch from train_loader 
        for batch_idx, sample in enumerate(tqdm(train_data_loader)):
            ...
            network_input_heads = []    #these are images of current batch
            network_input_heads.append(sample["image_rgb_input"].cuda())

            #training labels can be either belief maps or kps wrt model output
